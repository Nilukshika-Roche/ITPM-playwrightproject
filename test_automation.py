...
def _resolve_path(p: str | None) -> str | None:
    ...
    # 1. Try relative to Current Working Directory (CWD)
    cwd_candidate = (Path.cwd() / path).resolve()
    if cwd_candidate.exists():
        return str(cwd_candidate)

    # 2. Try relative to Script Directory (ROOT_DIR)
    root_candidate = (ROOT_DIR / path).resolve()
    if root_candidate.exists():
        return str(root_candidate)

    # 3. Try relative to a "test_automation" folder
...
def run_test():
    ...
    # Pre-check: Verify the file is writable before starting
    try:
        wb.save(args.output)
    except Exception as e:
        print("\n" + "!"*60)
        print("CRITICAL ERROR: Cannot save to the Excel file!")
        print(f"Details: {e}")
        print("PROBABLE CAUSE: The file is open in Excel. PLEASE CLOSE EXCEL.")
        print("!"*60 + "\n")
        return
